...
bool GoogleOAuth2::parsePayload(String& payload)
{
    #if defined(ESP8266)
...
String GoogleOAuth2::gzipInflate( const String& compressedBytes ) {
  String uncompressedBytes;

  unsigned full_length = compressedBytes.length() ;
  unsigned half_length = compressedBytes.length() / 2;
...
String GoogleOAuth2::readParam(const char * keyword)
{
